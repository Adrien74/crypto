...
public class PieChart_AWT_Node extends ApplicationFrame 
{
	String title;
	double[] donnees;
   public PieChart_AWT_Node( String title, double[] nbNodeResult) 
   {
      super( title ); 
      this.title = title;
   ...
   private PieDataset createDatasetForNbNode() 
   {
      ...
      dataset.setValue("Noeuds pour rumeur", result0);
      dataset.setValue("Noeuds pour contre rumeur", result1);
      dataset.setValue("Noeuds neutres", result2);        
      return dataset;         
   }
   ...
   private JFreeChart createChart( PieDataset dataset )
   {
      JFreeChart chart = ChartFactory.createPieChart(      
         this.title + ", total noeuds = " + this.donnees[3],  // chart title 
         dataset,        // data    
         true,           // include legend   
         true, 
         false);
